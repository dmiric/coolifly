...
class Modellers {
   ...
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, Modeller $plugin_manager_modeller, Event $plugin_manager_event, Actions $action_services, Conditions $condition_services, LoggerChannelInterface $logger, FileSystemInterface $file_system, StorageInterface $export_storage, ConfigFactoryInterface $config_factory, EventDispatcherInterface $event_dispatcher) {
    ...
    if ($events === NULL) {
      ...
      foreach ($this->pluginManagerEvent->getDefinitions() as $plugin_id => $definition) {
        try {
          ...
          $plugin->setTokens($this->getSupportedTokens($plugin));
          $events[] = $plugin;
        }
        catch (PluginException | \ReflectionException | \Throwable) {
          // Can be ignored.
        }
      }
   ...
   */
  private function getSupportedTokens(EventInterface $plugin): array {
    $sources = $this->eventDispatcher->getListeners(EcaEvents::BEFORE_INITIAL_EXECUTION);
    array_unshift($sources, [$plugin::class, 'buildEventData']);
    array_unshift($sources, [$plugin::class, 'getData']);

    $eventClass = $plugin->eventClass();
    $tokens = [];
