...
class FormEvent extends EventBase {
  ...
  #[Token(
    ...
    properties: [
      ...
      new Token(name: 'entity-bundle', description: 'The bundle of the form entity.'),
      new Token(name: 'entity-id', description: 'The form entity ID, only available if the entity is not new.'),
      new Token(name: 'entity-type', description: 'The form entity type.'),
      new Token(name: 'field-name', description: 'The name of the field.'),
      new Token(name: 'parent', description: 'The parent entity, if one exists.'),
      new Token(name: 'parent-bundle', description: 'The bundle of the parent entity, if one exists.'),
      new Token(name: 'parent-id', description: 'The ID of the parent entity, if one exists and it is not new.'),
      new Token(name: 'parent-type', description: 'The type of the parent entity, if one exists'),
  ...
  protected function buildEventData(): array {
    ...
    if ($event instanceof InlineEntityFormBuild) {
      ...
      if ($parent) {
        $data['parent-type'] = $parent->getEntityTypeId();
        $data['parent-bundle'] = $parent->bundle();
        if (!$parent->isNew()) {
